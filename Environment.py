...
class Environment:
	def __init__(self, width = WINDOWWIDTH, height = WINDOWHEIGHT) -> None:
		self.width = width
		self.height = height
		self.DISPLAYSURF = pygame.Surface((width, height))
		...
		self.score              = 0
		self.level, self.fall_freq   = calc_level_and_fall_freq(self.score)
	...
	def step(self):
		...
		for event in pygame.event.get():
			...
			if (event.type == KEYUP):
				...
				if (event.key == K_LEFT or event.key == K_a) and \
					is_valid_position(self.board, self.falling_piece, adj_X=-1):
		...
		if self.moving_down and time.time() - self.last_movedown_time > MOVEDOWNFREQ and is_valid_position(self.board, self.falling_piece, adj_Y=1):
			self.falling_piece['y'] += 1
			self.last_movedown_time = time.time()
		...
		if time.time() - self.last_fall_time > self.fall_freq:
			...
			if (not is_valid_position(self.board, self.falling_piece, adj_Y=1)):
				...
				num_removed_lines = remove_complete_lines(self.board)
